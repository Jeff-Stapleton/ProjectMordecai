...
*Converted from attack_taxonomy_v1.docx*

## Attack Taxonomy

v1.0.0 Last updated: 2026-03-11
...
Self-origin: spawns at character position (e.g., stomp).

Attached aura: follows the character for duration (e.g., storm field).

Ground-targeted: player becomes rooted, moves a reticle, releases to cast.
...
slightly wider arc sweep

forward step + sweep finisher
...
Build candidate directions around aim forward:

0°, ±2°, ±4°, ±6° (tunable)

Raycast each direction out to `AssistRange`.

If valid targets hit:

choose best by smallest angle offset, then closest target.

Slightly rotate firing direction toward chosen candidate before spawning projectile.

If no candidates, fire straight.
...
Self-origin: spawns at player instantly.

Attached aura: follows player, ticks/pulses.

Ground-targeted: enter targeting mode (rooted), move reticle, release to cast.


#### 6.2 Ground targeting controls

Controller: press skill → enter reticle mode; move reticle with Right Stick; press A to cast, press B to cancel.

KBM: press skill → enter reticle mode; reticle follows mouse; left-click to cast, right-click to cancel.
...
Spear (5 thrust chain + charged lunge)

One projectile (Longbow charged pierce)

One ground AoE (bom+ optional aura AoE
