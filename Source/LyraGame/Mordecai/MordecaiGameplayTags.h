// Project Mordecai — Native Gameplay Tag Declarations (US-002)

#pragma once

#include "NativeGameplayTags.h"

namespace MordecaiGameplayTags
{
	// State tags (US-003)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Airborne);

	// State tags (US-004)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Attacking);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_ComboWindow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Rooted);

	// State tags (US-005)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Dodging);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_PerfectDodge);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_DodgeCooldown);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_PostureBroken);

	// State tags (US-006)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Blocking);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_PerfectBlock);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_GuardBroken);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Parrying);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Parried);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Riposte);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_ParryWhiff);

	// State tags (US-007)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_KnockedDown);

	// Stamina tier tags (US-008)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stamina_Tier_Green);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stamina_Tier_Yellow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stamina_Tier_Red);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stamina_Tier_Exhausted);

	// State tags (US-008)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Sprinting);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_StaminaRegenPaused);

	// Status effect tags (US-013)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Burning);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Bleeding);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Poisoned);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Frostbitten);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Shocked);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Weakened);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Brittle);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Silenced);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Rooted);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Blinded);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Fear);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Cursed);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Exposed);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Corroded);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Drenched);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Focused);

	// Status effect category tags (US-013)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Category_DoT);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Category_Debuff);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Category_Control);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Category_Buff);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Category_Environmental);

	// Immunity tags (US-013)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Burning);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Bleeding);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Poisoned);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Frostbitten);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Shocked);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Weakened);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Brittle);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Silenced);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Rooted);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Blinded);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Fear);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Cursed);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Exposed);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Corroded);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Drenched);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Focused);

	// Status tags (US-015)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Frozen);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_MicroStunned);

	// Immunity tags (US-015)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_Frozen);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Immunity_MicroStunned);

	// State tags (US-014)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Casting);

	// State tags (US-019)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_CastingSlow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_CastingRooted);

	// Ability tags (US-019)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell);

	// Ability tags (US-020)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Fireball);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_ConeOfCold);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_MagicMissile);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_StoneSkin);

	// Status tags (US-020)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_StoneSkin);

	// Ability tags (US-021)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Blink);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Sleep);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_FireWard);

	// Status tags (US-021)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Sleeping);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_FireWard);

	// State tags (US-021)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Invulnerable);

	// Ability tags (US-022)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Bless);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Restoration);

	// Status tags (US-022)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Blessed);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Restoration);

	// Ability tags (US-023)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Snare);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Enfeeble);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_EnchantWeapon);

	// Status tags (US-023)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_EnchantedWeapon);

	// Event tags (US-019)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_SpellCast);

	// SetByCaller data tags (US-019)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(SetByCaller_SpellPointCost);

	// State tags (US-050)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(State_Dead);

	// Event tags (US-006)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_ParryInput);

	// Event tags (US-011)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_SkillMilestone);

	// Event tags (US-012)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_FeatUnlocked);

	// Event tags (US-014)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_CastInterrupted);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_DamageTaken);

	// Event tags (US-050)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_Death);

	// Event tags (US-053)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_PlayerDeath);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_EnemyKill);

	// SetByCaller data tags (US-004)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(SetByCaller_HealthDamage);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(SetByCaller_PostureDamage);

	// SetByCaller data tags (US-008)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(SetByCaller_StaminaCost);

	// Team tags (US-009)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Team_Enemy);

	// Attack tags (US-016)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Attack_Heavy);

	// Ability category tags (US-017)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Category_Spell);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Category_Magical);

	// Status tags (US-059)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Resolved);

	// Ability tags (US-060)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Illusion);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ability_Spell_Blur);

	// Status tags (US-060)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Status_Blurred);

	// Team tags (US-060)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Team_Ally);

	// Event tags (US-018)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_PerfectAction);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_NonPerfectAction);

	// Event tags (US-017)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Event_BreakFree);

	// Item type tags (US-032)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_Weapon);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_Armor);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_Trinket);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_Consumable);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_Material);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_TownResource);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_UpgradeKey);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_QuestItem);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Type_MagicalItem);

	// Item rarity tags (US-032)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Common);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Green);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Blue);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Purple);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Red);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Rarity_Gold);

	// Item identification tags (US-033)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Identification_Identified);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Item_Identification_Unidentified);

	// Weapon type tags (US-024)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Sword);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Axe);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Mace);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Spear);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Staff);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Dagger);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Bow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Crossbow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Wand);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Type_Unarmed);

	// Weapon slot tags (US-024)
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Slot_MainHand);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Slot_OffHand);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weapon_Slot_TwoHand);

	// Damage type tags (AC-002.11)
	// Physical subtypes are nested under Physical per damage_types_v1.md
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Physical);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Physical_Slash);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Physical_Pierce);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Physical_Blunt);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Fire);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Frost);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Lightning);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Poison);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Corrosive);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Arcane);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Shadow);
	LYRAGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage_Radiant);
}
